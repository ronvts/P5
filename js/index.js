...
class List {
  constructor(element, urlAPI) {
    this.ul = document.getElementById(element); //Sélection de la balise ul 
    ajaxGet(urlAPI).then ((products) => { //Retourne une promesse
      products.forEach(product => { //Pour chaque produit, on fait appel a la fonction renderHTML
        this.ul.appendChild(this.renderHTML(product)); //
      })
    }).catch(function (error) { //Si la requête ajax échoue
